...
# 后台IP设置
export Ipv4_ipaddr="192.168.1.249"            # 修改openwrt后台地址(填0为关闭)
...
export DNS_Settings="192.1686.1.1"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
# 更换固件内核
export Replace_Kernel="5.15.150"                    # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
...
export amlogic_kernel="5.15.01_6.1.01"
